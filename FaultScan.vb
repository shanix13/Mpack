...
Public Class FaultScan
    ...
    Dim picpassword As String = ""
    Public Sub checklogin(ByVal _ScanPIC As String)

        Dim loginapp As Boolean = False

        If txtScanPIC.Text.Contains(Chr(13)) Then
            Me.lbl_msg.Text = Nothing

            If _ScanPIC.Trim.ToLower = "display" Then
                loginapp = True
            ElseIf CheckUserAuth(_ScanPIC.Trim) = True Then
                loginapp = True
                'here
                frmMain.timBuzzer.Enabled = False
            End If
            ...
            If loginapp = True Then
                ...
                '      ResetDgvPreset()
                'End If
                SupPIC = picname ' txtScanPIC.Trim
                If errMsgLog <> "" Then
                    frmMain.SaveErrorText(errMsgLog, Now().ToString("yyyyMMdd") & "_errlog.txt")
                End If
        ...
        Using fs As New FileStream("Auth.txt", FileMode.Open)
            Using sr As New StreamReader(fs)
                ...
                While sr.Peek() > -1
                    ...
                    Dim user() As String
                    If dataline.Contains(",") Then
                        user = dataline.Split(New String() {","}, StringSplitOptions.None)
                        picpassword = user(0)
                        picname = user(1)
                    End If

                    'If dataline.ToLower.Trim = scanvalue Then
                    If picpassword = scanvalue.Trim Then
                        matchfound = True
                        Exit While
                    End If
                End While
